...
class AttackBuffer:
    ...
    def log_buffer(self, tokenizer, output, target, focus_score):
        message = "buffer:"
...
class GCG:
    def __init__(
        ...
        self.not_allowed_ids = None if config.allow_non_ascii else get_nonascii_toks(tokenizer, device=model.device, is_struq=False)
    ...
    def run(
        ...
        self.data = data
        self.instruction_len = len(self.tokenizer.encode(self.instruction))
        data_pre, data_post = data.split("{optim_str}")
        self.data_len = self.tokenizer(data_pre, add_special_tokens=False, return_tensors="pt")["input_ids"].shape[1] + \
                   self.tokenizer(data_post, add_special_tokens=False, return_tensors="pt")["input_ids"].shape[1]
            ...
            for step in tqdm(range(config.num_steps)):
                ...
                with torch.no_grad():
                    ...
                    if self.prefix_cache:
                        ...
                        input_embeds_detection = torch.cat([
                            embedding_layer(sampled_ids),
                            after_embeds.repeat(new_search_width, 1, 1),
                        ], dim=1)
                    else:
                    ...
                    self.data_len_all = self.data_len + optim_ids.shape[1]
                    loss = find_executable_batch_size(self.compute_candidates_loss, batch_size)(input_embeds, input_embeds_detection)
                    # print(loss.shape)
                    # exit(0)
                ...
                tmp_data = data.replace("{optim_str}", optim_str)
                tmp_instruction = instruction
                is_injection, detect_result = self.detector.detect_with_instruction(tmp_instruction, tmp_data)
                tmp_messages = [
                    {"role": "system", "content": tmp_instruction},
                    {"role": "user", "content": "\nText: " + tmp_data}
                ]
                input = tokenizer.apply_chat_template(tmp_messages, add_generation_prompt=True, return_tensors="pt").to(model.device)
                output = model.generate(input, do_sample=False, max_new_tokens=32)
                output = tokenizer.batch_decode(output[:, input.shape[1]:], skip_special_tokens=True)[0]

                buffer.log_buffer(tokenizer, output, target, detect_result['focus_score'])                

                if check_success(output, target, injected_task) and not is_injection:
                    self.stop_flag = True
                    success = True
                
                if step % 10 == 0 or success:
                    log_step = {"step": step, "loss": current_loss, "success": success, "focus_score": detect_result['focus_score'], "optim_str": optim_str, "output": output, "target": target}
    ...
    def init_buffer(self) -> AttackBuffer:
        ...
        self.data_len_all = self.data_len + init_optim_ids.shape[1]
        ...
        if self.prefix_cache:
            ...
            init_buffer_embeds_detection = torch.cat([
                self.embedding_layer(init_buffer_ids),
                self.after_embeds.repeat(true_buffer_size, 1, 1),
            ], dim=1)
        else:
        ...
        init_buffer_losses = find_executable_batch_size(self.compute_candidates_loss, true_buffer_size)(init_buffer_embeds, init_buffer_embeds_detection)
    ...
    def compute_token_gradient(
        ...
        # data_len = self.data_len + optim_ids.shape[1]
        if self.prefix_cache:
            input_embeds_detection = torch.cat([optim_embeds, self.after_embeds], dim=1)
            focus_score = self.detector.detect_gcg(self.instruction_len, self.data_len_all, input_embeds_detection, prefix_cache_batch=self.prefix_cache)[0]
        else:
            input_embeds_detection = torch.cat([self.before_embeds, optim_embeds, self.after_embeds], dim=1)
            focus_score = self.detector.detect_gcg(self.instruction_len, self.data_len_all, input_embeds_detection)[0]
        loss = loss - self.config.alpha * focus_score
    ...
    def compute_candidates_loss(
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                input_embeds_detection_batch = input_embeds_detection[i:i+search_batch_size]
                ...
                if self.prefix_cache:    
                    if not prefix_cache_batch or current_batch_size != search_batch_size:
                        prefix_cache_batch = [[x.expand(current_batch_size, -1, -1, -1) for x in self.prefix_cache[i]] for i in range(len(self.prefix_cache))]
                    focus_score = self.detector.detect_gcg(self.instruction_len, self.data_len_all, input_embeds_detection_batch, prefix_cache_batch)[0]
                else:
                    focus_score = self.detector.detect_gcg(self.instruction_len, self.data_len_all, input_embeds_detection_batch)[0]
...
def run(
    ...
    gcg = GCG(model, tokenizer, detector, config)
    result = gcg.run(instruction, data, target, injected_task)
    return result
