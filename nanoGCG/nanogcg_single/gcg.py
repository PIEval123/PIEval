...
class GCG:
    def __init__(
        ...
        is_struq = config.is_struq
        self.not_allowed_ids = None if config.allow_non_ascii else get_nonascii_toks(tokenizer, device=model.device, is_struq=is_struq)
    ...
    def run(
        ...
        with open(config.saved_path, mode="w", encoding="utf-8") as f:
            for step in tqdm(range(config.num_steps)):
                ...
                output = model.generate(input, do_sample=False, max_new_tokens=20)
                output = tokenizer.batch_decode(output[:, input.shape[1]:], skip_special_tokens=True)[0]

                buffer.log_buffer(tokenizer, output, target)                

                if check_success(output, target, injected_task):
                    self.stop_flag = True
                    success = True
                
                if step % 10 == 0 or success:
                    log_step = {"step": step, "loss": current_loss, "success": success, "optim_str": optim_str, "output": output, "target": target}
